...
export class DomAnimatedSpriteObject extends DomGameObject {
    #frames = [];
    #frameDuration;
    #textureScale = 1;
    #elapsedTime = 0;
    ...
    _afterRender(dt) {
        if (this.#frameDuration === undefined || !this.#atlas || this.#frames.length === 0)
            return;
        const lastIndex = this.#frames.length - 1;
        if (!this.#loop && this.#currentFrameIndex === lastIndex)
            return;
        this.#elapsedTime += dt;
